...
import Data.ByteString qualified as P (ByteString)
...
import Data.ByteString.Lazy.Internal qualified as BI
...
import System.IO qualified as IO (stdin, stdout)
...
intersperse w (Chunk c cs)
  ...
  where
    intersperse' :: P.ByteString -> P.ByteString
  ...
  where
    ...
    takeWhile' (Chunk c cs) =
        case findIndexOrEnd (not . f) c of
            0 -> Empty ()
            n
                | n < B.length c -> Chunk (B.take n c) (Empty ())
...
toStreamingByteStringWith strategy builder0 = do
    ...
    loop cios
{-# INLINEABLE toStreamingByteStringWith #-}
