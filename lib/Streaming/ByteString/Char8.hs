        ...
        case text of
            ...
            Chunk c cs ->
                case nthNewLine c (n - counter) of
                    ...
                    Right !l ->
                        Chunk (B.unsafeTake l c) $
                            Empty $
                                loop1 $!
                                    Chunk (B.unsafeDrop l c) cs
{-# INLINEABLE lineSplit #-}
...
readInt = start
  where
    ...
    start bs@(Chunk c cs)
        ...
        | w <- B.unsafeHead c =
            if
                    | w - 0x30 <= 9 -> readDec True Nothing bs
                    | let rest = Chunk (B.tail c) cs ->
                        if
                                | w == 0x2b -> readDec True (Just w) rest
